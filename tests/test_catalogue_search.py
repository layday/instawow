...
async def test_basic_search(iw_manager: Manager):
    limit = 5
    results = await search(iw_manager, 'molinari', limit=limit)
...
async def test_search_flavour_filtering(iw_manager: Manager):
    results = await search(iw_manager, 'atlas loot classic', limit=10)
...
async def test_search_source_filtering(iw_manager: Manager):
    results = await search(iw_manager, 'molinari', limit=5, sources={'curse'})
...
async def test_search_date_filtering(iw_manager: Manager):
    ...
    results = await search(iw_manager, 'molinari', limit=5, start_date=start_date)
    assert all(e.last_updated > start_date for e in results)


async def test_search_unknown_source(iw_manager: Manager):
    with pytest.raises(ValueError, match='Unknown source'):
        await search(iw_manager, 'molinari', limit=5, sources={'foo'})


async def test_search_filter_installed(iw_manager: Manager):
    results = await search(iw_manager, 'molinari', limit=5, filter_installed='include_only')
    assert not results
    ...
    results = await search(iw_manager, 'molinari', limit=5)
    ...
    results = await search(iw_manager, 'molinari', limit=5, filter_installed='exclude')
    assert {('curse', 'molinari'), ('github', 'p3lim-wow/molinari')} & {
        (e.source, e.slug) for e in results
    } == {('github', 'p3lim-wow/molinari')}

    results = await search(
        iw_manager, 'molinari', limit=5, filter_installed='exclude_from_all_sources'
    )
    assert {('curse', 'molinari'), ('github', 'p3lim-wow/molinari')} & {
        (e.source, e.slug) for e in results
    } == set()
...
async def test_search_prefer_source(iw_manager: Manager):
    results = await search(iw_manager, 'molinari', limit=5, prefer_source=None)
    assert {('curse', 'molinari'), ('github', 'p3lim-wow/molinari')} <= {
        (e.source, e.slug) for e in results
    }

    results = await search(iw_manager, 'molinari', limit=5, prefer_source='github')
    assert {('curse', 'molinari'), ('github', 'p3lim-wow/molinari')} & {
        (e.source, e.slug) for e in results
    } == {('github', 'p3lim-wow/molinari')}
