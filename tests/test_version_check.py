...
from instawow import _version_check
...
async def test_is_outdated_works_in_variety_of_scenarios(
    ...
    with monkeypatch.context() as patcher:
        patcher.setattr(_version_check, 'get_version', lambda: '0+dev')
        assert await _version_check.is_outdated(global_config) == (False, '')

    # Update check disabled, version not cached
    assert await _version_check.is_outdated(
        attrs.evolve(global_config, auto_update_check=False)
    ) == (False, '')

    # Endpoint not responsive, version not cached
    with monkeypatch.context() as patcher:
        patcher.setattr(_version_check, 'get_version', lambda: '0.1.0')
        iw_aresponses.add(
            **Route(
        ...
        assert await _version_check.is_outdated(global_config) == (False, '0.1.0')
    ...
    with monkeypatch.context() as patcher:
        ...
        assert await _version_check.is_outdated(global_config) == (True, '1.0.0')
    ...
    with monkeypatch.context() as patcher:
        patcher.setattr(_version_check, 'get_version', lambda: '1.0.0')
        iw_aresponses.add(
            **Route(
                '//pypi.org/simple/instawow',
                {'versions': ['1.0.0']},
        ...
        assert await _version_check.is_outdated(global_config) == (False, '1.0.0')
